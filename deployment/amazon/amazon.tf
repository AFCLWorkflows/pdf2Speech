...
# Upload deployment package for split function
resource "aws_s3_object" "split_deployment_package" {
  bucket = aws_s3_bucket.deployment_bucket.id
  key    = "split.jar"
  acl    = "private"  # or can be "public-read"
  source = "${path.root}/split/target/deployable/split-1.0-SNAPSHOT.jar"
  etag   = filemd5("${path.root}/split/target/deployable/split-1.0-SNAPSHOT.jar")
}

# Upload deployment package for extract function
resource "aws_s3_object" "extract_deployment_package" {
  bucket = aws_s3_bucket.deployment_bucket.id
  key    = "extract.jar"
  acl    = "private"  # or can be "public-read"
  source = "${path.root}/extract/target/deployable/extract-1.0-SNAPSHOT.jar"
  etag   = filemd5("${path.root}/extract/target/deployable/extract-1.0-SNAPSHOT.jar")
}
...
resource "aws_lambda_function" "extract_function" {
  ...
  s3_bucket        = aws_s3_object.extract_deployment_package.bucket
  s3_key           = aws_s3_object.extract_deployment_package.key
  runtime          = "java11"
  handler          = "function.ExtractFunction::handleRequest"
  source_code_hash = filebase64sha256(aws_s3_object.extract_deployment_package.source)
  role             = data.aws_iam_role.lab_role.arn
  timeout          = 500
  memory_size      = 512
  ephemeral_storage { size = 512 }
...
resource "aws_cloudwatch_log_group" "split_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.split_function.function_name}"
  retention_in_days = 14
}

resource "aws_cloudwatch_log_group" "extract_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.extract_function.function_name}"
  retention_in_days = 14
}
